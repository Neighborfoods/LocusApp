...
import { BorderRadius, FontSize, FontWeight, Spacing } from "@theme/index";
...
const HOUSING_REASONS = [
  {
    ...
    description: "Lost my job or income",
  },
  {
    value: "loss_of_loved_one",
    icon: "heart-broken",
    description: "Lost someone close to me",
  },
  {
    value: "high_mortgage_rates",
    icon: "trending-up",
    description: "Mortgage rates became unaffordable",
  },
  {
    value: "divorce",
  ...
  },
  {
    value: "exploring",
    icon: "compass",
    description: "I'm just exploring the concept",
  },
] as const;
        ...
        <View key={i} style={si.row}>
          <View style={[si.circle, i <= current - 1 && si.circleActive]}>
            {i < current - 1 ? (
              ...
              <Text style={[si.num, i === current - 1 && si.numActive]}>
                {i + 1}
              </Text>
            )}
          </View>
          {i < total - 1 && (
            <View style={[si.line, i < current - 1 && si.lineActive]} />
          )}
        </View>
      ))}
    </View>
...
const createStepIndicatorStyles = (
  colors: ReturnType<typeof import("@theme/colors").getColors>,
) =>
  StyleSheet.create({
    container: {
...
export default function RegisterScreen() {
  ...
  const handleStep3 = step2Form.handleSubmit(async (data) => {
    ...
    forceSkipTimerRef.current = setTimeout(
      () => setShowForceSkip(true),
      FORCE_SKIP_THRESHOLD_MS,
    );
    setError(null);
    ...
    try {
      ...
      setError(msg);
      setUserExistsEmail(
        isUserExists ? e?.prefillEmail ?? step1Data?.email ?? null : null,
      );
      shakeError();
      if (isUserExists && (e?.prefillEmail ?? step1Data?.email)) {
        navigation.navigate("Login", {
          prefillEmail: e?.prefillEmail ?? step1Data?.email ?? undefined,
        });
      }
    } finally {
    ...
    >
      ...
      >
            ...
            <View style={styles.errorBannerContent}>
              ...
              {userExistsEmail ? (
                <TouchableOpacity
                  onPress={() =>
                    navigation.navigate("Login", {
                      prefillEmail: userExistsEmail,
                    })
                  }
                  style={styles.loginInsteadBtn}
                >
                  <Text
                    style={[styles.loginInsteadText, { color: colors.primary }]}
                  >
                    Log in instead
                  </Text>
                </TouchableOpacity>
        ...
        {step === 2 && (
          <View style={styles.stepContainer}>
            <Text style={[styles.stepLabel, { color: colors.text }]}>
              What brought you to Locus?
            </Text>
            <Text style={[styles.stepHint, { color: colors.textSecondary }]}>
              This is confidential and helps us tailor your experience.
            </Text>
          ...
          <View style={styles.stepContainer}>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Field
                    ...
                    value={field.value}
                    onChangeText={field.onChange}
                    error={step2Form.formState.errors.confirm_password?.message}
                    secureTextEntry={!showPw}
            ...
            >
              ...
              >
                ...
                Policy. A background check consent will be required before
            ...
            {(showForceSkip || (__DEV__ && step === 3)) && (
              ...
              >
                <Text style={[styles.forceSkipText, { color: colors.danger }]}>
                  Force Skip to App (debug)
                </Text>
              </TouchableOpacity>
            )}
            {__DEV__ && (
              ...
              >
                <Text style={[styles.forceSkipText, { color: colors.primary }]}>
                  Skip to Map (Dev Only)
                </Text>
              </TouchableOpacity>
            )}
          </View>
